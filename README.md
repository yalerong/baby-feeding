...
- 修改或删除辅食记录会回退它自动打的那次卡（`food_trials.logSources` 按日期记录每天是哪条记录打的卡，`lastLogRecordId` 为最后一天的快照；解锁页手动打的卡、同一天还有别的记录吃到该食材、或同日记录拉取失败时都不回退）；补录昨天及更早的辅食也会打卡，但早于该食材最近一次试吃日期的补录不打。已知限制：删掉连续试吃中间某一天的记录不会重算天数，需要到解锁页手动撤销。
